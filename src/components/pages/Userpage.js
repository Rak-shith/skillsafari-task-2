...
const Userpage = () => {
  ...
  const loadUsers = async () => {
    const result = await axios.get(`https://skillsafari-task2.herokuapp.com/users`);
  ...
  const deleteUser = async id => {
    await axios.delete(`https://skillsafari-task2.herokuapp.com/users/${id}`);
    loadUsers();
  };
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    return await axios
    .get(`https://skillsafari-task2.herokuapp.com/users?q=${value}`)
    .then((response) => {
  ...
  const handleSort = async (e) => {
    ...
    setSortValue(value);
    return await axios
    .get(`https://skillsafari-task2.herokuapp.com/users?_sort=${value}&_order=asc`)
    .then((response) => {
      setUser(response.data);
    })
    .catch((err) => console.log(err));
