...
const EditUser = () => {
  ...
  const onSubmit = async e => {
    e.preventDefault();
    await axios.put(`https://skillsafari-task2.herokuapp.com/users/${id}`, loggedInUser);
    if(loggedInUser){
  ...
  const loadUser = async () => {
    const result = await axios.get(`https://skillsafari-task2.herokuapp.com/users/${id}`);
